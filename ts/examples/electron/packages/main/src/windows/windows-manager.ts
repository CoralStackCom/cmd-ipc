...
import { MessagePortMainChannel } from '../commands/message-port-main-channel'
import { LogManager } from '../logging/log-manager'
import { Environment } from '../utils/environment'
import { WindowEventIDs } from './window-events'
import type {
...
class WindowManagerClass {
  ...
  private _initialiseWindow(id: string, window: BrowserWindow, directIPCChannel: boolean = false) {
    ...
    window.webContents.on('did-finish-load', () => {
      ...
      window.webContents.postMessage(WindowEventIDs.SET_LOGGING_LEVEL, LogManager.getLogLevel())

      // Connect process to Main process via new IPC Channel
      const { port1, port2 } = new MessageChannelMain()
      window.webContents.postMessage(WindowEventIDs.REGISTER_NEW_CHANNEL, ChannelIDs.MAIN, [port1])
      ...
      if (directIPCChannel) {
        this._windows
          ...
          .forEach((win) => {
            ...
            window.webContents.postMessage(WindowEventIDs.REGISTER_NEW_CHANNEL, win.id, [
              directPort1,
            ])
            win.window.webContents.postMessage(WindowEventIDs.REGISTER_NEW_CHANNEL, id, [
              directPort2,
            ])
          })
      }
        ...
        this._windows
          ...
          .forEach((process) => {
            if (process.id !== id) {
              ...
              const { port1: directPort1, port2: directPort2 } = new MessageChannelMain()
              window.webContents.postMessage(WindowEventIDs.REGISTER_NEW_CHANNEL, process.id, [
                directPort1,
              ])
              process.window.webContents.postMessage(WindowEventIDs.REGISTER_NEW_CHANNEL, id, [
                directPort2,
              ])
            }
          })
