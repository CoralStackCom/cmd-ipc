...
Typed events — the event payload struct *is* the event. Attach `#[event("id")]` to a struct that also derives `Serialize` / `Deserialize` / `JsonSchema` and it becomes a typed event usable with `emit` / `on`.
...
- `#[event("id")]` — attribute on a payload struct that derives `Serialize` / `Deserialize` / `JsonSchema`. Emits an `impl Event for Struct` with id and schemars-derived schema. The struct is then usable as `registry.emit(Struct { .. })` and `registry.on::<Struct>(cb)`. Use a `///` doc comment on the struct for human-readable descriptions.
