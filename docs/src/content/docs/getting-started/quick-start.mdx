...
<Tabs syncKey="lang">
  ...
  <TabItem label="Rust" icon="seti:rust">
    Rust doesn't have a separate schema map like the TypeScript version. Any type you use in a command handler just needs to derive `Serialize`, `Deserialize`, and `JsonSchema` — the registry generates the wire-level schema from those derives automatically.
    ...
    With these types defined, you can now use them in the command handler in [Step 3](#step-3-create-command-handlers).
  </TabItem>
</Tabs>
