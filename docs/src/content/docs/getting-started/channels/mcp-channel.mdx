...
MCP channels bridge <a href="https://modelcontextprotocol.io/" target="_blank" rel="noopener">Model Context Protocol</a> with cmd-ipc. Both the TypeScript and Rust implementations delegate protocol handling — JSON-RPC, sessions, streaming, and (for TS) OAuth — to the official MCP SDKs.
...
<Tabs syncKey="lang">
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    `@coralstack/cmd-ipc` is a peer dependency and must also be installed. The package wraps the official <a href="https://www.npmjs.com/package/@modelcontextprotocol/sdk" target="_blank" rel="noopener">@modelcontextprotocol/sdk</a>.
  </TabItem>
  <TabItem label="Rust" icon="seti:rust">
    ...
    The crate wraps the <a href="https://docs.rs/rmcp" target="_blank" rel="noopener">`rmcp`</a> SDK. Only the server side (`McpServerChannel`) is implemented today — the client side (connecting to remote MCP servers from Rust) is not yet available.
  </TabItem>
</Tabs>

## Rust: quick path

For the common case of exposing a Rust `CommandRegistry` as an MCP server over stdio (e.g. for Claude Desktop), you only need a few lines:
...
Public commands (non-`_`-prefixed) appear to the MCP client via `tools/list`; `tools/call` routes through the registry's normal dispatch. Private commands are never exposed.

The rest of this page covers the TypeScript API in detail.
