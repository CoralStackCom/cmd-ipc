...
<Aside type="note">
  `InMemoryChannel` is Rust-only. The TypeScript package uses `MessagePortChannel` with `MessageChannel()` for the equivalent in-process use case.
</Aside>
...
let driver_root = root.register_channel(ch_for_root as Arc<dyn CommandChannel>).await?;
let driver_worker = worker.register_channel(ch_for_worker as Arc<dyn CommandChannel>).await?;
