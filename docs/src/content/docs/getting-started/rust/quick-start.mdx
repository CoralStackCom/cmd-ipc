...
Events work the same way — emit on either side, listen on either side:
...
- **Private commands** (prefixed with `_`) stay local to the registering process.
- **Events** broadcast to all connected registries with no response expected.
