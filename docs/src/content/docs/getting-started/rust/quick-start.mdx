...
<Aside type="tip">
  `coralstack-cmd-ipc` re-exports `serde` and `schemars`, which the `#[payload]` and `#[event]` macros expand against. That's why they don't appear in `Cargo.toml` — you only list `serde_json` here because the quick-start pokes at raw JSON values directly.
</Aside>
...
Strict mode (`execute::<calc_service::Add>(..)`) and loose mode (`execute_dyn("calc.add", ..)`) cover both ends of the typing spectrum. See [Type Safety](/cmd-ipc/getting-started/rust/type-safety/) for the tradeoffs and when to reach for each.
