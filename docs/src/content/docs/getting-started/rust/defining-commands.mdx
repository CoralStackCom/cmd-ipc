...
description: Register commands in Rust with the #[command] / #[commands] macros
---

import { Aside } from '@astrojs/starlight/components';

A command is an async function with an ID. The `#[command]` attribute macro binds a function (or impl method) to a command ID, and `#[commands]` on an `impl` block lets you register every tagged method at once via `MyService.register_all(&registry)`.
